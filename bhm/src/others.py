def cleaning(catalogue: fits, save: bool, **kwargs) -> dict:
    ...
    fluxes = np.asarray(
        [data[CONFIG.flux.value_cols[i]] for i in range(CONFIG.nband)]
    ).T
    fluxes_err = np.asarray(
        [data[CONFIG.flux.err_cols[i]] for i in range(CONFIG.nband)]
    ).T
    ...
    df_flux_err = pd.DataFrame(
        fluxes_err, columns=CONFIG.flux.err_cols, dtype=np.float32
    )
